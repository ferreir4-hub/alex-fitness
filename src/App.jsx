...
export default function App() {
  ...
  const [goal, setGoal] = useState(() => safeParse("goal", 2));
  const [water, setWater] = useState(() => safeParse("water", 0));
  const [days, setDays] = useState(() => safeParse("days", {}));

  const [exerciseLog, setExerciseLog] = useState(() => safeParse("exerciseLog", {}));
  const [exercise, setExercise] = useState("");
  const [selectedDay, setSelectedDay] = useState(today);

  const [notes, setNotes] = useState(() => safeParse("notes", []));
  const [newNote, setNewNote] = useState("");

  const [challenges, setChallenges] = useState(() => safeParse("challenges", []));
        ...
        <div className="bg-white rounded-3xl shadow-lg p-5">
          <div className="flex justify-between mb-2">
            ...
            <div className="flex gap-2">
              <input value={newChallenge} onChang
